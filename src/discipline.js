...
// evs is timestamp-sorted (groupSessions guarantees this). Returns the timestamp of the
// FIRST event that marks this session as gm-driven, or null if it never touches gm --
// a boolean would let a late gm-spool touch retroactively flag every earlier tool call
// in the same session, even ones made hours before gm was ever invoked.
function gmSessionStartTs(evs) {
  for (const ev of evs) {
    const b = ev.block || {};
    if (b.type !== 'tool_use') continue;
    if (b.name === 'Skill' && b.input?.skill === 'gm') return ev.timestamp;
    if (b.name === 'Write' && GM_SPOOL_RE.test(String(b.input?.file_path || ''))) return ev.timestamp;
    if (b.name === 'Bash' && GM_SPOOL_RE.test(String(b.input?.command || ''))) return ev.timestamp;
  }
  return null;
}
...
export function gitDiscipline(rows, maxSamples = 10, manual = false) {
  ...
  for (const evs of sessions.values()) {
    const gmStartTs = gmSessionStartTs(evs);
    ...
    for (const ev of evs) {
      ...
      if (gmStartTs !== null && ev.timestamp >= gmStartTs && GIT_COMMIT_RE.test(cmd)) gmRawGit.push(sample(ev, cmd, manual));
...
const FIND_LEADING_RE = /^\s*(cd\s+\S+\s*&&\s*)?find\s/;

function targetsOutsideCwd(cwd, targetPath) {
  if (!cwd || !targetPath) return false;
  const normCwd = path.resolve(cwd).replace(/\\/g, '/').toLowerCase();
  const normTarget = path.resolve(cwd, targetPath).replace(/\\/g, '/').toLowerCase();
  return !normTarget.startsWith(normCwd);
}

function isExemptKnownPathLookup(toolName, input, cwd) {
  ...
  if (targetPath && targetsOutsideCwd(cwd, targetPath)) return true;
  return false;
}

function findCommandTarget(cmd) {
  const m = cmd.match(/^\s*(?:cd\s+(\S+)\s*&&\s*)?find\s+(\S+)/);
  if (!m) return null;
  return m[1] || m[2] || '.';
}
...
export function searchDiscipline(rows, maxSamples = 10, manual = false) {
  ...
  for (const evs of sessions.values()) {
    const gmStartTs = gmSessionStartTs(evs);
    if (gmStartTs === null) continue;
    gmSessions++;
    const cwd = evs[0]?.conversation?.cwd || '';
    for (const ev of evs) {
      if (ev.timestamp < gmStartTs) continue;
      const b = ev.block || {};
      if (b.type !== 'tool_use') continue;
      if (b.name === 'Grep' || b.name === 'Glob') {
        if (isExemptKnownPathLookup(b.name, b.input, cwd)) { exempted++; continue; }
        observations.push(sample(ev, `${b.name} ${b.input?.pattern || ''}`, manual));
        continue;
      }
      if (b.name === 'Bash' || b.name === 'PowerShell') {
        const cmd = String(b.input?.command || '');
        if (!FIND_LEADING_RE.test(cmd)) continue;
        const target = findCommandTarget(cmd);
        if (target && targetsOutsideCwd(cwd, target)) { exempted++; continue; }
        observations.push(sample(ev, `${b.name} ${cmd} (use codesearch mode=filename)`, manual));
      }
    }
  }
  process.stdout.write(`# search-discipline: ${sessions.size} sessions, ${gmSessions} gm sessions, ${exempted} exempt known-path/outside-cwd lookups skipped\n`);
  report('Grep/Glob/find discovery inside gm session (prefer codesearch verb)', observations, maxSamples);
  ...
  for (const evs of sessions.values()) {
    ...
    for (const ev of evs) {
      ...
      if (b.name === 'WebFetch' || b.name === 'WebSearch') {
        ...
        const desc = String(b.input?.description || '');
        const subagentType = String(b.input?.subagent_type || '');
        if (subagentType !== 'fork' && !/\bgm\b.*\bskill\b|\/gm\b|Skill\(skill=.gm.\)/i.test(prompt)) {
          observations.push(sample(ev, `${b.name} ${desc.slice(0, 80)} (dispatch omits 'use the gm skill' -- subagent defaults to raw Grep/Glob/find)`, manual));
        }
      } else if (b.name === 'Bash') {
        const cmd = String(b.input?.command || '');
...
export function spoolDiscipline(rows, maxSamples = 10, manual = false) {
  ...
  let gmSessions = 0;
  for (const evs of sessions.values()) {
    if (gmSessionStartTs(evs) === null) continue;
    gmSessions++;
